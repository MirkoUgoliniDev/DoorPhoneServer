#!/bin/bash
# rollback.sh — Rimuove tutto ciò che il wizard DoorPhoneServer ha installato
# Lascia intatto il repository git e i tool di sistema preesistenti.
#
# Uso: rollback.sh [--yes]
#   --yes  Salta tutte le conferme interattive (modalità non-interattiva per la WebUI)
...
_remove_generated=false
if $YES_ALL; then
  _remove_generated=true
else
  read -p "  Confermi rimozione? [s/N] " -n 1 -r; echo
  [[ $REPLY =~ ^[SsYy]$ ]] && _remove_generated=true
fi

if $_remove_generated; then
...
# python3-flask, python3-tk, python3-dotenv non vengono rimossi:
# sono necessari al wizard webui per funzionare.
...
if [ ${#TO_REMOVE[@]} -gt 0 ]; then
    ...
    if $YES_ALL; then
      ...
      [[ $REPLY =~ ^[SsYy]$ ]] && _remove_pkgs=true
    fi

    if $_remove_pkgs; then
